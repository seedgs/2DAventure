...
public class Record : MonoBehaviour
{
    ...
    //在script创建文件夹（Enemy）
    //在Enemy文件夹内创建文件（Boar）
    //在Boar文件夹创建新的#C文件，命名为Boar
    //在Boar脚本内，把MonoBehaviour改为Enemy（目的是为了让Boar脚本可以继承Enemy脚本的方法与类）
    //在Enemy文件夹内创建文件#C文件，命名为Enemy
    //Enemy内编译，创建Animator为anim
    //Enemy内编译，创建Rigidbody2D为rb
    //Enemy内编译，创建float属性normalSpeed
    //Enemy内编译，创建float属性currentSpeed
    //Enemy内编译，创建float属性chaseSpeed
    //Enemy内编译，创建Vector3类的faceDir
    //因为野猪现在的方向（transform的scale的x数值为1，palyer的transform的scale的x数值也为1）但是两个物体的朝向却是反的，所以需要需要数值统一
    //在Update()内，把人物与野猪的数值统一（也就是faceDir的x数值为负数）
    //在Awake()，添加rb组件、anim组件、同时currentSpeed需要等于normalSpeed
    //创建方法Move()
    //把rb的Velocity数值与faceDir、CurrentSpeed关联

    //在Unity面板中，Animation文件夹内创建Enemy文件、Boar文件夹
    //Boar文件夹内创建动画控制组件（Animator Controller），命名为Boar
    //在Unity面板中，野猪Boar需要加载组件Animator，并关联（Animator Controller）的Boar组件
    //Unity面板的Animation中，创建野猪的播放动画（包括奔跑、步行）（注意切割动画图片的设置！！！）
    //Unity面板的Animator（注意要选中Boar！！！，否则还会停留在Player或者其他Layer层上面）
    //Animator中，创建bool属性的isRun、bool属性的isWalk
    //Animator中，可视化组件中关联相关选项
    //Enemy脚本内，Move()方法中，public后添加Virtual（目的是为了允许子类修改Move()方法）
    //Boar脚本内，创建Override的Move()方法（目的是允许Boar修改Move()方法）
    //Boar脚本-Move()方法内，添加关联动画代码
}
